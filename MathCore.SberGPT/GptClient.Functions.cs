...
public partial class GptClient
{
    ...
    public record FunctionInfo(
        ...
        Dictionary<string, (string Name, string? Description, string Type, IEnumerable<string>? Variants)> Parameters,
        [property: JsonPropertyName("return_type")]
        (string Type, IEnumerable<string>? Variants) ReturnType
    )
    {
    ...
    private static FunctionInfo GetFunctionInfo(Delegate function)
    {
        ...
        var @params = info
            ...
            .ToDictionary(
                ...
                p =>
                {
                    var parameter_name = p.ParameterType.Name;
                    if (p.GetCustomAttribute<FunctionParameterNameAttribute>() is { } attr)
                        parameter_name = attr.Name;
...
file class FunctionExamples
{
    ...
    public static double GetTemperature(string location, DateTime time)
    {
    ...
    public static IEnumerable<FileInfo> GetFile(string Path, string Mask, bool Recurrent)
    {
        return Directory
    ...
    public static string GetBinaryFileContentInBase64(string FilePath)
    {
